...
class MainActivity : ComponentActivity() {
    ...
    private fun assetFilePath(assetName: String): String {
        ...
        try {
            ...
            if (finalSize == 0L) {
                val errorMsg = "Asset file $assetName was copied but resulting file is empty (0 bytes). Asset may be missing or corrupted."
                Log.e(debugTag, errorMsg)
                throw IOException(errorMsg)
            }
            
            Log.d(debugTag, "Successfully copied asset $assetName: $totalBytesCopied bytes -> $finalSize bytes")
            return file.absolutePath
        } catch (e: IOException) {
            Log.e(debugTag, "Error copying asset $assetName: ${e.message}")
            // Delete the file if it exists but is empty
    ...
    private fun loadCifar10Data(
        ...
        try {
            ...
            val cifar10Dir = "cifar-10/extracted_data"
            ...
            for (batchFile in trainBatchFiles) {
                ...
                if (!file.exists()) {
                    Log.e(debugTag, "File does not exist: $internalPath")
                    throw IOException("File does not exist: $internalPath")
                }
                val totalSize = file.length()
                val totalRecords = (totalSize / recordSize).toInt()
                
                Log.d(debugTag, "File: $batchFile, Size: $totalSize bytes, Records: $totalRecords, Expected record size: $recordSize")

                // Calculate how many images to load from this batch
                val imagesToLoad = minOf(imagesPerBatch, trainSize - trainImagesLoaded, totalRecords)
                
                if (imagesToLoad <= 0) {
                    Log.e(debugTag, "No images to load from $batchFile. Total records: $totalRecords, Requested: ${trainSize - trainImagesLoaded}")
                    break
                }
                
                if (totalSize < imagesToLoad * recordSize) {
                    Log.e(debugTag, "File too small: $totalSize bytes, need ${imagesToLoad * recordSize} bytes")
                    throw IOException("File too small: $totalSize bytes, need ${imagesToLoad * recordSize} bytes")
                }
                ...
                DataInputStream(FileInputStream(file)).use { input ->
                    for (i in 0 until imagesToLoad) {
                        try {
                            ...
                            Log.e(debugTag, "EOFException at image $i in $batchFile: ${e.message}")
                            throw IOException("Unexpected end of file while reading image $i from $batchFile", e)
                        }
                    }
                }
            ...
            for (batchFile in testBatchFiles) {
                ...
                // Calculate how many images to load from this batch
                val imagesToLoad = minOf(imagesPerBatch, testSize - testImagesLoaded, totalRecords)
                
                if (imagesToLoad <= 0) {
                    Log.e(debugTag, "No images to load from $batchFile. Total records: $totalRecords, Requested: ${testSize - testImagesLoaded}")
                    break
                }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Apply test transformations to the data (normalization with CIFAR-10 mean/std)
        ...
        try {
            ...
            val dataSize = dataFile.length()

            Log.d(
                debugTag,
                "Model file exists: ${modelFile.exists()}, readable: ${modelFile.canRead()}, size: $modelSize bytes"
            )
            Log.d(
                debugTag,
                "Data file exists: ${dataFile.exists()}, readable: ${dataFile.canRead()}, size: $dataSize bytes"
            )

            // Validate file sizes
            if (!modelFile.exists() || modelSize == 0L) {
                val errorMsg = "Model file is missing or empty: $modelPath (size: $modelSize bytes)"
                Log.e(debugTag, errorMsg)
                val errorTextView: TextView = findViewById(R.id.resultTextView)
                errorTextView.text = "Error: Model file is missing or empty"
                return
            }

            if (!dataFile.exists() || dataSize == 0L) {
                val errorMsg = "Data file is missing or empty: $dataPath (size: $dataSize bytes). Please ensure generic_cifar.ptd is generated and placed in assets folder."
                Log.e(debugTag, errorMsg)
                val errorTextView: TextView = findViewById(R.id.resultTextView)
                errorTextView.text = "Error: Data file (generic_cifar.ptd) is missing or empty. Please generate it using the Python script."
                return
            }
            ...
            e.printStackTrace()
            val errorTextView: TextView = findViewById(R.id.resultTextView)
            errorTextView.text = "Error loading model: ${e.message}"
            // Don't finish() - let user see the error message
        }

        // Log input tensor information for debugging
        Log.d(debugTag, "Input image tensor shape: ${batchTestImageTensor.shape().joinToString(", ")}")
        Log.d(debugTag, "Input label tensor shape: ${testLabelBuffer.shape().joinToString(", ")}")
        Log.d(debugTag, "Input image tensor dtype: ${batchTestImageTensor.dtype()}")
        Log.d(debugTag, "Input label tensor dtype: ${testLabelBuffer.dtype()}")
        
        // Verify tensor data is valid
        val imageData = batchTestImageTensor.getDataAsFloatArray()
        val labelData = testLabelBuffer.getDataAsLongArray()
        Log.d(debugTag, "Image tensor data range: min=${imageData.minOrNull()}, max=${imageData.maxOrNull()}")
        Log.d(debugTag, "Label tensor data: ${labelData.joinToString(", ")}")
        
        // Skip the initial test execution in onCreate - let user trigger it via buttons
        // This allows the app to start successfully and user can test via Evaluate button
        Log.d(debugTag, "Skipping initial model execution in onCreate. Use Evaluate button to test the model.")
    ...
    private fun evaluateModel(
        ...
        for (batch in 0 until num_test_batches) {
            ...
            Log.d(debugTag, "Batch $batch: Image tensor shape: ${batchTestImageTensor.shape().joinToString(", ")}, Label shape: ${testLabelBuffer.shape().joinToString(", ")}")
            
            val outputEValues = try {
                model.executeForwardBackward("forward", *inputEValues)
                    ?: throw IllegalStateException("Execution module is not loaded.")
            } catch (e: Exception) {
                Log.e(debugTag, "Error in batch $batch: ${e.message}", e)
